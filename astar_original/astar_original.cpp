...
void create_sh(string pasta, string dominio, string problema, int num_problema, string heuristic, int numDominio) {
	...
	arquivo = "astar_original/"+heuristic+"/problemas/" + arquivo;
	...
	outfile<<"RESULTS=/home/marvin/marvin/astar_original/"<<heuristic<<"/problemas/"<<pasta.c_str()<<"/resultado"<<"\n\ncd /home/marvin/fd\n\n";
	...
	outfile<<"src/search/downward-release --domain_name "<<pasta.c_str()<<" --problem_name "<<problema.c_str()<<" --heuristic_name "<<heuristic<<" --search \"astar_original("<<heuristic<<"())\" <  "<<sas.c_str()<<" >> ${RESULTS}/"<<problema.c_str()<<"\n\n";
...
void entrada_dados(string &pasta, string &problema, string &dominio, bool &dominio_unico, int &quantidade_problemas) {
	...
	ifstream file2("h/astar_original/instance360.txt");
	...
	while (counter < total_heuristics) {
		...
		ifstream file("h/astar_original/d/instance360.txt");
		...
		for (int i = 0; i < quantidade_entrada_opt; i++) {
			...
			string pastaProblema = "mkdir /home/marvin/marvin/astar_original/"+heuristic+"/problemas/"+pasta;
			//string pastaProblema = "mkdir ~/astar_original/"+heuristic+"/problemas/"+pasta;
			printf("Tenta criar a pasta dominio.\n");
			system(pastaProblema.c_str());
			string pastaResultado = "mkdir /home/marvin/marvin/astar_original/"+heuristic+"/problemas/"+pasta+"/resultado";
		
			//string pastaResultado = "mkdir ~/astar_original/"+heuristic+"/problemas/"+pasta+"/resultado";
