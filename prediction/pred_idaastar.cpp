...
void create_kre_report(string Ni, string fdist, string idai, string heuristic, string pasta, string fileName) {
	//Ni = A* + blind (Dijkstra)	
	...
	//Call ida* instead of A* because ss counts duplicates and A* does not count duplucates, then  the comparation
	//must be ida*-bfs ss.
	...
	ifstream fidai(idai.c_str());
	fidai>>str; //title
	fidai>>str; //initial heuristic label
	fidai>>str; //initial heuristic value
	fidai>>str; //time label
	fidai>>str; //bound label
	fidai>>str; //expanded nodes label
	fidai>>str; //generated nodes label
	totallevels4 = getTotalLevels(idai.c_str());
        cout<<"totallevels4 = "<<totallevels4<<endl;

	levels4 = new string*[totallevels4];

	for (int i = 0; i < totallevels4; i++) {
	    levels4[i] = new string[4];
	}
	...
	for (int i = 0; i < totallevels4; i++) {
            v_time.push_back(levels4[i][0]);
	    v_bound.push_back(atof(levels4[i][1].c_str()));
	    v_exp.push_back(atof(levels4[i][2].c_str()));
	    v_gen.push_back(atof(levels4[i][3].c_str()));
	}
	fidai.close();

	//Write in prediction report directory.
	string output;

	output = fileName;    
        output =  pasta+"/"+output;
	output = "prediction/"+heuristic+"/pre_ida/"+output;
	...
	for (int i = 0; i < vpred.size(); i++) {
            cout<<"i = "<<i<<endl;
            cout<<"v_bound "<<i<<" = "<<v_bound.at(i)<<endl;
            cout<<"v_exp "<<i<<" = "<<v_exp.at(i)<<endl;
 	    cout<<"vpred.at "<<i<<" = "<<vpred.at(i)<<endl;
          

	    outputFile<<"\t"<<v_bound.at(i)<<"\t\t"<<v_exp.at(i)<<"\t\t"<<vpred.at(i)<<"\n";
	}
...
void create_report1(string heuristic, string blind, int countProblems) {
	...
	do {
		...
		readFile>>pasta;
                ...
                string directorySS = "mkdir /home/marvin/marvin/prediction/"+heuristic+"/pre_ida";
		if (system(directorySS.c_str())) {
		   cout<<"Directory pre_ida created."<<endl;
		}
 
		string  pastaReporte = "mkdir /home/marvin/marvin/prediction/"+heuristic+"/pre_ida/"+pasta;
		...
		//Read the files to get the number of nodes by level of Dijkstra = A* + blind
                //Read the files from dijkstra/dijkstra/reportdijkstra/
        	string output;
		output =  pasta+"/"+output;
		...
		//Read the files that contains the execution of the problem using A* + ipdb
                //Read the files from ida/ipdb/reportastar 

		string outputida;
                outputida =  pasta+"/"+outputida;
		outputida = "idai/"+heuristic+"/reportidai/"+outputida;
		outputida = "marvin/" + outputida;
		outputida = "marvin/" + outputida;
		outputida = "/home/" + outputida;	
	     

		DIR *dir4;
        	struct dirent *ent4;
        
       	 	dir4 = opendir(outputida.c_str());
        	if (dir4 != NULL) {
	    		...
	    		cout<<"Error trying to open the directory: "<<outputida.c_str()<<endl;
	...
	//Send  only the files that match the files in the directories.
		...
		for (int i = 0; i < fileNames.size(); i++) {
			...
			for (int k = 0; k < fileNames3.size(); k++) {
				...
				idai = outputida+fileNames4.at(z);
				string four = fileNames4.at(z);
				if ((one == three) && (one == four) && (three == four)) {	
                                   cout<<"one = "<<one<<endl; 
				   create_kre_report(Ni.c_str(), fdist.c_str(), idai.c_str(), heuristic, pasta, one);
			    	}
			    }
             		}
		}
