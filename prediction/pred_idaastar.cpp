...
void create_kre_report(string Ni, string unusedbf, string fdist, string idai, string heuristic, string pasta, string fileName) {
	...
	//Call A* + consistent heuristic (e,g. merge_and_shrink) - Information to compare.
	int totallevels4;
	...
	output = fileName;    
        output =  pasta+"/"+output;
	output = "prediction/"+heuristic+"/reportss/"+output;
...
void create_report1(string heuristic, string blind, int countProblems) {
	...
	do {
		...
		readFile>>pasta;
                ...
                string directorySS = "mkdir /home/marvin/marvin/prediction/"+heuristic+"/reportss";
		if (system(directorySS.c_str())) {
		   cout<<"Directory reportss created."<<endl;
		}
 
		string  pastaReporte = "mkdir /home/marvin/marvin/prediction/"+heuristic+"/reportss/"+pasta;
		...
		for (int i = 0; i < fileNames.size(); i++) {
			...
			for (int k = 0; k < fileNames3.size(); k++) {
				...
				if ((one == three) && (one == four) && (three == four)) {	
                                   cout<<"one = "<<one<<endl; 
				   create_kre_report(Ni.c_str(), "nada", fdist.c_str(), idai.c_str(), heuristic, pasta, one);
			    	}
			    }
             		}
		}
