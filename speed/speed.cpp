...
void create_sh(string pasta, string dominio, string problema, int num_problema, string heuristic, int numDominio) {
	...
	arquivo = "speed/"+heuristic+"/problemas/" + arquivo;
	...
	outfile<<"#PBS -N speed_d"<<numDominio<<"_p"<<(num_problema+1)<<"\n\n#PBS -m b\n\n#PBS -M marvin.zarate@ufv.br\n\ncd $PBS_O_WORKDIR\n\nsource /usr/share/modules/init/bash\n\nmodule load python\nmodule load mercurial\n\n";
	//outfile<<"ulimit -v 6500000\n\n"; //SET LIMIT 6GB
        //PBS -l walltime=200

	cout<<"pasta = "<<pasta.c_str()<<"\n\n";
	outfile<<"RESULTS=/home/marvin/marvin/speed/"<<heuristic<<"/problemas/"<<pasta.c_str()<<"/resultado"<<"\n\ncd /home/marvin/fd\n\n";
	...
	outfile<<"src/search/downward-release --domain_name "<<pasta.c_str()<<" --problem_name "<<problema.c_str()<<" --heuristic_name "<<heuristic<<" --search \"astar_speed("<<heuristic<<"())\" <  "<<sas.c_str()<<" > ${RESULTS}/"<<problema.c_str()<<"\n\n";
...
void entrada_dados(string &pasta, string &problema, string &dominio, bool &dominio_unico, int &quantidade_problemas) {
	...
	ifstream file2("h/speed/instance360.txt");
	...
	while (counter < total_heuristics) {
		...
		ifstream file("h/speed/d/instance360.txt");
		...
		for (int i = 0; i < quantidade_entrada_opt; i++) {
			...
			string pastaProblema = "mkdir /home/marvin/marvin/speed/"+heuristic+"/problemas/"+pasta;
			//string pastaProblema = "mkdir ~/speed/"+heuristic+"/problemas/"+pasta;
			printf("Tenta criar a pasta dominio.\n");
			system(pastaProblema.c_str());
			string pastaResultado = "mkdir /home/marvin/marvin/speed/"+heuristic+"/problemas/"+pasta+"/resultado";
		
			//string pastaResultado = "mkdir ~/speed/"+heuristic+"/problemas/"+pasta+"/resultado";
