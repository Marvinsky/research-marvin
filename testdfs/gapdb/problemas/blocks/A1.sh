...
src/search/downward-release --global_probes 100 --domain_name blocks --problem_name probBLOCKS-4-0.pddl --heuristic_name gapdb  --search "dfs_culprits(min([gapdb(mp=0.5), gapdb(mp=0.5, disjoint=true)  ]))" <  Astarblocks1 >> ${RESULTS}/probBLOCKS-4-0.pddl
