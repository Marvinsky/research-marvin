...
void create_sh(string pasta, string dominio, string problema, int num_problema, string heuristic, int numDominio) {
	...
	arquivo = "testdfs/"+heuristic+"/problemas_culprits/" + arquivo;
	...
	outfile<<"RESULTS=/home/marvin/marvin/testdfs/"<<heuristic<<"/problemas_culprits/"<<pasta.c_str()<<"/resultado"<<"\n\ncd /home/marvin/fd-culprits\n\n";
	...
	outfile<<"src/search/downward-release --global_probes 100 --domain_name "<<pasta.c_str()<<" --problem_name "<<problema.c_str()<<" --heuristic_name "<<heuristic<<"  --search \"dfs_culprits(max([ipdb(), merge_and_shrink()]))\" <  "<<sas.c_str()<<" >> ${RESULTS}/"<<problema.c_str()<<"\n\n";
...
void entrada_dados(string &pasta, string &problema, string &dominio, bool &dominio_unico, int &quantidade_problemas) {
	...
	while (counter < total_heuristics) {
		...
		for (int i = 0; i < quantidade_entrada_opt; i++) {
			...
			string pastaProblema = "mkdir /home/marvin/marvin/testdfs/"+heuristic+"/problemas_culprits/"+pasta;
			//string pastaProblema = "mkdir ~/testdfs/"+heuristic+"/problemas_culprits/"+pasta;
			printf("Tenta criar a pasta dominio.\n");
			system(pastaProblema.c_str());
			string pastaResultado = "mkdir /home/marvin/marvin/testdfs/"+heuristic+"/problemas_culprits/"+pasta+"/resultado";
		
			//string pastaResultado = "mkdir ~/testdfs/"+heuristic+"/problemas_culprits/"+pasta+"/resultado";
