...
//probBLOCKS-9-2_F_44.csv -> probBLOCKS-9-2
string convertToDomain(string aux) {
	string t1 = aux;
	size_t f1 = t1.find("F");
	string domain;
	if (f1 > 0) {
		domain = t1.substr(0, f1 - 1);
	}
	return domain;
}
...
void create_report1(string heuristic, string algorithm1, string algorithm2, int countProblems) {
	...
	do {
		...
		for (iter_m = map_bc_file_astar.begin(); iter_m != map_bc_file_astar.end(); iter_m++) {
			...
			//cout<<"astarBC = "<<astarBC<<"\n";
			string domain_astarBC = convertToDomain(astarBC);
			//cout<<"domain_astarBC = "<<domain_astarBC<<"\n";
			...
			for (size_t j = 0; j < v_files_ss_bc_set.size(); j++) {
				...
				//cout<<"\t\tssBC = "<<ssBC<<"\n";
				string domain_ssBC = convertToDomain(ssBC);
				//cout<<"domaon_ssBC = "<<domain_ssBC<<"\n";
				string output_ssBC = output + ssBC;
				//cout<<"output_ssBC = "<<output_ssBC<<"\n\n";
				
				//Here we should add a new validation about:
				// 1.- Consider that the number of nodes generated by executed A* using the heuristics always must be compared with the number of nodes generated by SS generated by the Last F_boundary
				// 2.- The F_boundary will change when the number of probes change.
