...
There are different approaches (Haslum et al., (\citeyear{haslum2007domain}); Edelkamp, (\citeyear{edelkamp2007automated}); Nissim et al., (\citeyear{nissim2011computing})) that have been developed to generate heuristics. The way to create heuristics could be based on the domain, it means that the context where the problem was created is known. Or without knowledge of the domain, where the heuristic lacks of the context. These systems are called Heuristic Generators by Barley et al., (\citeyear{BarleySantiagoOver}). Actually, the approache that have showed most successfull results in heuristic generation is the Pattern Database (\texttt{PDBs}). The way how \texttt{PDBs} works is the following: The search space of the problem is abstracted into a smaller state space that can be enumerated with exhaustive search. The distance of all abstracted states to the abstracted goal state are stored in a lookup table, which can be used as a heuristic function for the original state space.
