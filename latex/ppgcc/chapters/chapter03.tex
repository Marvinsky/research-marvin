...
Where $\Psi(\zeta\sp{'},\nabla)$ is an objective function and $N$ is the desired subset size. $N$ could be determined by a hard constraint such as the maximum number of \texttt{PDBs} one can store in memory. According to Rayner; Sturtevant; Bowling, (\citeyear{raynersss13}) it is unlikely that there is an efficient algorithm for solving Equation \ref{eq:equationmin}. We use an algorithm based on the work of Buchbinder et al., (\citeyear{buchbinder2014submodular}) we call Random Greedy Heuristic Selection (\texttt{RGHS}) to approximately solve Equation \ref{eq:equationmin} for different functions $\Psi$.\\
...
The work of Rayner; Sturtevant; Bowling, (\citeyear{raynersss13}) uses a greedy algorithm introduced by Nemhauser; Wolsey; Fisher, (\citeyear{nemhauser1978analysis}) for approximately solving the heuristic subset selection problem. In contrast with the \texttt{RGHS} presented above, which is based on the work of Buchbinder et al., (\citeyear{buchbinder2014submodular}), Nemhauser; Wolsey; Fisher, (\citeyear{nemhauser1978analysis})'s approach does not offer near$-$optimality guarantees when the problem's objective functions is non$-$monotone. As mentioned before A$\sp{*}$'s running time is a non$-$monotone objective function. While \texttt{RGHS} also offers guarantees for non$-$monotone objective functions, it retains the same guarantees offered by Nemhauser; Wolsey; Fisher, (\citeyear{nemhauser1978analysis})'s algorithm when optimizing monotone objective functions Buchbinder et al., (\citeyear{buchbinder2014submodular}). That is why we only consider Buchbinder et al., (\citeyear{buchbinder2014submodular})'s approach in our theoretical and experimental analyses.
...
Lemma \ref{le:lemmaone} and \ref{le:lemmatwo} are sufficient for using a result by Buchbinder et al., (\citeyear{buchbinder2014submodular}) to conclude the following.
...
The assumption that $t_{h_{i}}\ =\ t_{h_{j}}$ for any $h_{i},h_{j}\ \in\ \zeta$ often does not hold in domain$-$independent planning. For example, the \texttt{iPDB} heuristic Haslum et al., (\citeyear{haslum2007domain}) can be few order of magnitude faster than the Incremental \texttt{LM-Cut} Helmert; Domshlak, (\citeyear{helmert2009landmarks}) heuristic. In order to lift such an assumption we first define A$\sp{*}$'s running time in terms of its computational cost as follows.\\
...
Since $T\sp{'}$ is supermodular, another result by Buchbinder et al., (\citeyear{buchbinder2014submodular}) and Lemma \ref{le:lemmathree} allow us to conclude the following.
