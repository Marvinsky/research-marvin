...
that minimize $\Psi(\zeta_{i-1}\sp{'} \cup \{h\})$ breaking ties arbitrarily.\\
	Let $h_{i}$ be greedily selected from $M_{i}$\\
...
Algorithm \ref{alg:ghs_algorithm} shows \texttt{GHS}. \texttt{GHS} receives as input a problem $\nabla$, a set of heuristics $\zeta$, and it returns a subset $\zeta\sp{'} \subseteq \zeta$. In each iteration $i$ \texttt{GHS} selects a heuristic $h_{i}$ from a pool $M_{i}$ of ``good‘’ heurisitcs and adds $h_{i}$ to $\zeta\sp{'}$. $M_{i}$ is defined as follows. $M_{i} \subseteq \zeta\ \backslash \zeta_{i-1}\sp{'}$ is a subset minimizing $\Psi(\zeta_{i-1}\sp{'} \cup \{h\})$ breaking ties arbitrarily, where $\zeta_{i-1}\sp{'}$ is the subset of heuristics \texttt{GHS} selects prior to iteration $i$ of the algorithm. $M_{i}$ contains heuristics that when individually combined with $\zeta_{i-1}\sp{'}$ minimizes $\Psi$ the most. \texttt{GHS} returns $\zeta\sp{'}$ when no more enhancement exists for the objective function.\\
