...
void create_sh(string pasta, string dominio, string problema, int num_problema, string heuristic, int numDominio) {
	...
	arquivo = "testkre/"+heuristic+"/problemas/" + arquivo;
	...
	outfile<<"RESULTS=/home/marvin/marvin/testkre/"<<heuristic<<"/problemas/"<<pasta.c_str()<<"/resultado"<<"\n\ncd /home/marvin/fd\n\n";
	...
	outfile<<"src/search/downward --search \"astarkre("<<heuristic<<"())\" <  "<<sas.c_str()<<" >> ${RESULTS}/"<<problema.c_str()<<"\n\n";
...
void entrada_dados(string &pasta, string &problema, string &dominio, bool &dominio_unico, int &quantidade_problemas) {
	...
	ifstream file2("h/kre/oneHeuristicTD-blocks.txt");
	...
	while (counter < total_heuristics) {
		...
		ifstream file("h/kre/d/todosdominios-blocks.txt");
		...
		for (int i = 0; i < quantidade_entrada_opt; i++) {
			...
			string pastaProblema = "mkdir /home/marvin/marvin/testkre/"+heuristic+"/problemas/"+pasta;
			//string pastaProblema = "mkdir ~/testkre/"+heuristic+"/problemas/"+pasta;
			printf("Tenta criar a pasta dominio.\n");
			system(pastaProblema.c_str());
			string pastaResultado = "mkdir /home/marvin/marvin/testkre/"+heuristic+"/problemas/"+pasta+"/resultado";
		
			//string pastaResultado = "mkdir ~/testkre/"+heuristic+"/problemas/"+pasta+"/resultado";
