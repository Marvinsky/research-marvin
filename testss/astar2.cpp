...
void create_sh(string pasta, string dominio, string problema, int num_problema, string heuristic, int numDominio) {
	...
	outfile<<"RESULTS=/home/marvin/marvin/testss/"<<heuristic<<"/problemas/"<<pasta.c_str()<<"/resultado2"<<"\n\ncd /home/marvin/fd\n\n";
	...
	//outfile<<"src/search/downward --global_probes 10000 --domain_name "<<pasta.c_str()<<" --problem_name "<<problema.c_str()<<" --heuristic_name "<<heuristic<<" --search \"ss2("<<heuristic<<"())\" <  "<<sas.c_str()<<" >> ${RESULTS}/"<<problema.c_str()<<"\n\n";
...
void entrada_dados(string &pasta, string &problema, string &dominio, bool &dominio_unico, int &quantidade_problemas) {
	...
	while (counter < total_heuristics) {
		...
		for (int i = 0; i < quantidade_entrada_opt; i++) {
			...
			string pastaResultado = "mkdir /home/marvin/marvin/testss/"+heuristic+"/problemas/"+pasta+"/resultado2";
