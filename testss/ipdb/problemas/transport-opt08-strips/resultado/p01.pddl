Dispatcher selected state size 1.
This is a nonunit task.
Estos son los inicios de los inicios.
argv[0] = src/search/downward-1
argv[0][0] = s
argv[1] = --global_probes
argv[2] = 1
argv[4] = transport-opt08-strips
argv[5] = --problem_name
argc = 11
Simplifying transitions... done!
OptionParser::parse_cmd_line
registered: ss
registered: ipdb
OptionParser::parse_cmd_line
registered: ss
registered: ipdb
Average operator cost: 3.69231
Memory usage before compute_additive_vars:1044
Memory usage after compute_additive_vars:1044
canonical_max_memory=205844
Memory usage after compute_max_cliques:1044
PDB collection construction time: 0s
Got 2 PDBs
Found 1 cliques
done calculating initial pattern collection and candidate patterns for the search
Calling iPDB with time_limit:0
current collection size is 10
current initial h value: 4
starting generating samples
	 starting improvement phase
pattern: [0, 4] - improvement: 27
pattern: [0, 5] - improvement: 31
pattern: [1, 4] - improvement: 188
pattern: [1, 5] - improvement: 172
found a better pattern with improvement 188
pattern: [1, 4]
canonical_max_memory=206888
Hill-climbing time so far: 0.01s
current collection size is 25
current initial h value: 4
starting generating samples
	 starting improvement phase
pattern: [0, 4] - improvement: 24
pattern: [0, 5] - improvement: 44
pattern: [1, 5] - improvement: 116
pattern: [0, 1, 4] - improvement: 963
found a better pattern with improvement 963
pattern: [0, 1, 4]
canonical_max_memory=207932
Hill-climbing time so far: 0.02s
current collection size is 70
current initial h value: 54
starting generating samples
	 starting improvement phase
pattern: [0, 5] - improvement: 31
pattern: [1, 5] - improvement: 39
found a better pattern with improvement 39
pattern: [1, 5]
canonical_max_memory=208976
Hill-climbing time so far: 0.18s
current collection size is 85
current initial h value: 54
starting generating samples
	 starting improvement phase
pattern: [0, 5] - improvement: 30
pattern: [0, 1, 5] - improvement: 242
found a better pattern with improvement 242
pattern: [0, 1, 5]
canonical_max_memory=210020
Hill-climbing time so far: 0.27s
current collection size is 130
current initial h value: 54
starting generating samples
	 starting improvement phase
Improvement below threshold. Aborting hill climbing.
Pruned 3 of 5 cliques
Pruned 2 of 6 PDBs
Dominance pruning took 0s
iPDB: iterations = 5
iPDB: num_patterns = 4
iPDB: size = 100
iPDB: improvement = 0
iPDB: generated = 18
iPDB: rejected = 0
iPDB: max_pdb_size = 225
Pattern generation (Haslum et al.) time: 0.37s
rg: 0.01
 ___________________________________
|         calling initialize        |
 ___________________________________
SSSearch ...
Initial heuristic value: 54
h_min: 54 depth: 0 #states: 0 time: 0s
arquivo = p01.pddl1
dominio = transport-opt08-strips
tarefa = p01.pddl
heuristica = ipdb

expansions = 1
Directory: ipdb created.
Directory: fdist created.
print.
g:0
size: 1
	f: 54	q: 1

g:1
size: 0

g:2
size: 0

g:3
size: 0

g:4
size: 0

g:5
size: 0

g:6
size: 0


expansions = 1
**********
p = 1
prePre = 1
**********
totalPrediction = 1
 ____________________________________
|   total numero of call step() = 0   |
 ____________________________________
Actual search time: 0s [t=0.37s]
Search time: 0s
Total time: 0.37s
Peak memory: 7548 KB
VmRSS memory: 1288 KB
VmHWM memory: 1288 KB
destructor of HST eliminates all the hashed state descriptions
destructor of HST eliminates all the hashed state descriptions
destructor of HST eliminates all the hashed state descriptions
destructor of HST eliminates all the hashed state descriptions
destructor of HST eliminates all the hashed state descriptions
