...
void create_sh(string pasta, string dominio, string problema, int num_problema, string heuristic, int numDominio) {
	...
	arquivo = "dijkstra/"+heuristic+"/problemas/" + arquivo;
	...
	outfile<<"RESULTS=/home/marvin/marvin/dijkstra/"<<heuristic<<"/problemas/"<<pasta.c_str()<<"/resultado"<<"\n\ncd /home/marvin/fd\n\n";
	...
	outfile<<"src/preprocess/preprocess < "<<sas.c_str()<<".sas"<<"\n\n";	
        ...
        //dijkstr-release
	outfile<<"src/search/downward-release --domain_name "<<pasta.c_str()<<" --problem_name "<<problema.c_str()<<" --heuristic_name "<<heuristic<<" --search \"astar_dijkstra("<<heuristic<<"())\" <  "<<sas.c_str()<<" > ${RESULTS}/"<<problema.c_str()<<"\n\n";
...
void entrada_dados(string &pasta, string &problema, string &dominio, bool &dominio_unico, int &quantidade_problemas) {
	...
	ifstream file2("h/dijkstra/instance360.txt");
	...
	while (counter < total_heuristics) {
		...
		ifstream file("h/dijkstra/d/instance360.txt");
		...
		for (int i = 0; i < quantidade_entrada_opt; i++) {
			...
			string pastaProblema = "mkdir /home/marvin/marvin/dijkstra/"+heuristic+"/problemas/"+pasta;
			//string pastaProblema = "mkdir ~/dijkstra/"+heuristic+"/problemas/"+pasta;
			printf("Tenta criar a pasta dominio.\n");
			system(pastaProblema.c_str());
			string pastaResultado = "mkdir /home/marvin/marvin/dijkstra/"+heuristic+"/problemas/"+pasta+"/resultado";
		
			//string pastaResultado = "mkdir ~/dijkstra/"+heuristic+"/problemas/"+pasta+"/resultado";
